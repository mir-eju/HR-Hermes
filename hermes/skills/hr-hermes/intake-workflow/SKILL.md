...
description: HR-Hermes email → Trello → Telegram/Slack approval → Gmail reply (run end-to-end for all active projects).
version: 1.0.0
metadata:
  hermes:
    tags: [hr-hermes, email, trello]
...
## 0) Safety
- Never call `mcp_gmail_send_reply` unless you have a valid `tokenId` from an `approved` signal (Slack or Telegram) and you pass the **exact** approved reply text.
...
5. If `not_a_requirement`, skip Trello + approval chat (Slack/Telegram) and close out the thread state appropriately via `update_thread`.
...
8. Post approval using **`mcp_firestore_get_project`** (or equivalent) to see which channels exist:
   - If `telegram` is configured (`telegram.chatId` present), call **`mcp_telegram_post_approval_message`**.
   - Else if `slack` is configured, call **`mcp_slack_post_approval_message`**.
   - If neither is configured, `append_error` and stop for this thread.
...
2. For each signal:
   ...
   - `edited` / `edit_reply`: delete the signal (guardrail already refreshed Slack or Telegram).
